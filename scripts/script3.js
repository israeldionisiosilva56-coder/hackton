...
const RECOMENDACOES_MAP = [
    { indice: 0, pontoRisco: 14, texto: 'Use um gerenciador de senhas e crie senhas únicas e fortes para cada conta.' },
    { indice: 1, pontoRisco: 14, texto: 'Sempre verifique o cadeado HTTPS antes de inserir dados em qualquer site.' },
    { indice: 2, pontoRisco: 14, texto: 'Desconfie de links recebidos por e-mail ou mensagens não solicitadas — phishing é real.' },
    { indice: 3, pontoRisco: 14, texto: 'Ative a autenticação em dois fatores (2FA) em todas as contas importantes.' },
    { indice: 4, pontoRisco: 14, texto: 'Mantenha sistema e aplicativos sempre atualizados para corrigir falhas de segurança.' },
    { indice: 5, pontoRisco: 14, texto: 'Evite divulgar informações pessoais sensíveis em redes sociais ou Wi-Fi público.' },
    { indice: 6, pontoRisco: 14, texto: 'Revise permissões antes de instalar apps — aplicativos maliciosos podem roubar seus dados.' }
];
...
function gerarRecomendacoes() {
    ...
    const itens = RECOMENDACOES_MAP.filter(m => respostas[m.indice] === m.pontoRisco);
...
document.getElementById('btnAvancar').addEventListener('click', function (e) {
    ...
    const interval = setInterval(() => {
        ...
        if (perc >= 100) {
            ...
            setTimeout(() => { window.location.href = 'pagina3.html'; }, 500);
        }
    }, 50);
});